...
#include "VFD.h"
//建構子
VFD::VFD(int CS_Pin, int MAX_RPM)
{
...
void VFD::VFD_DATA_RX()
{
  ...
  if (RX_DATA_Flag == 1)
  {
    for (rx_count = 0; rx_count < RS485_RX_SIZE; rx_count++)
    { //驅動器接收中斷資料
      RX_Data[rx_count] = RX_Data_buf[rx_count];
    } //end or (i=0;i<SIZE_RX;i++){

    for (rx_count = 0; rx_count < 8; rx_count++)
    { //驅動器左輪接收中斷資料LWeel_DATA(資料分割)
      DATA_RX_ID1[rx_count] = RX_Data[rx_count];
    } //end or (i=0;i<SIZE_RX;i++){

    for (rx_count = 0; rx_count < 8; rx_count++)
    { //驅動器右輪接收中斷資料RWeel_DATA(資料分割)
...
void VFD::MS_TO_RPM()
{
  ...
  //R
  if (linear_vel_x_R >= MAX_RPM_) //SPEED_LIMIT-正轉
